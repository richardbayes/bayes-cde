...
ALLPREDS = zeros(mcmc_samples,nt); % RDP -- get predictive distribution
SIG2 = zeros(mcmc_samples,1); % RDP sigma2 samples
ALLMEANS = zeros(mcmc_samples,nt); % RDP
CENTERS = cell(mcmc_samples,1); % RDP - the centers of the tessellation
...
while sample < mcmc_samples
   ...
   if count>burn_in
      ...
      aaaa = aaa + normrnd(0,sqrt(sig2),nt,k); % RDP adds predictive distribution
      pred_mean = zeros(nt,1);
      pred_sample = zeros(nt,1);
      pred_sample_predictive = zeros(1,nt); % RDP adds predictive distribution
      for jj=1:k
         indx = find(W_test==jj);
         pred_mean(indx) = aa(indx,jj);
         pred_sample(indx)=aaa(indx,jj);
         pred_sample_predictive(indx) = aaaa(indx,jj); % RDP adds predictive distribution
      end
      ...
      ALLPREDS(sample,:) = pred_sample_predictive; % RDP
      ALLMEANS(sample,:) = pred_sample'; % RDP
      SIG2(sample) = sig2; % RDP
      CENTERS{sample} = Theta(1:k,:) .* sx + mx;
