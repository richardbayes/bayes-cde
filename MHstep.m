function [r,T,accepted] = MHstep(y,X,T,gpgeneral,Mmax,n,n_min,precision,Z)
    ...
    % Calculate the prior (needed for parallel tempering)
    p = size(X,2);
    T.lprior = -log(p) - log(Mmax) + lchoose(n,T.M);
end
